...
function help(){
   echo "
Ejecución de un acceptanceTest
Uso:
    $0 <enviromentId> [deploymentTableFile] [--help]


PARÁMETROS:
    <enviromentId>  Entorno sobre el que se ejecutará el acceptanceTest
...
function executeTest(){
   ...
   for acceptanceTest in `cat $0|grep "^function acceptanceTest.*()"\
      ...
      errorCode=$?
      if [ "$errorCode" == "0" ]; then
         echo "acceptanceTest[$acceptanceTest]  Success"
      else 
         echo "acceptanceTest[$acceptanceTest]  Fail"
         failed=true
      fi
   done;
